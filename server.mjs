...
const IMAGE_DIR = path.join(__dirname, "public", "planets");
...
function scanBodyImages() {
  ...
  for (const filename of files) {
    ...
    if (!NAME_MAP[slug]) continue; // 지원 목록 외 파일은 스킵
    const { ko, type } = NAME_MAP[slug];
    bodies.push({
      slug,
      name_ko: ko,
      type, // '행성', '항성', '위성', '왜소행성', '소천체'
      image: `/public/planets/${filename}`
    });
  }
  return bodies;
}
...
console.log(`[boot] 발견한 천체 이미지: ${BODY_BANK.length}개`);
if (BODY_BANK.length) {
  console.log("[boot] 샘플:", BODY_BANK.slice(0, 4));
}

// ========== 사진 퀴즈 생성 ==========
// 정답과 같은 '분류(type)' 우선으로 오답을 뽑고, 부족하면 전체에서 보충
...
app.post("/chat", (req, res) => {
  try {
    const mode = req.body?.mode || "photo"; // 기본을 photo로
    let quiz = null;

    if (mode === "photo") {
      quiz = makePhotoQuiz();
    } else {
      // 다른 모드는 아직 필요 없으면 photo로 폴백
      quiz = makePhotoQuiz();
    }
